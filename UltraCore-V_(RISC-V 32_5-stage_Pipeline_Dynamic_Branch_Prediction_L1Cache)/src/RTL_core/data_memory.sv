...
module data_memory #(
  ...
  always_comb begin
    ...
    if (dmem_req && !dmem_wr_en) begin
      case (dmem_data_size)
        BYTE: begin
          dmem_rd_data = dmem_zero_extend ? {24'b0, mem[addr]} : {{24{mem[addr][7]}}, mem[addr]};
        end

        HALF_WORD: begin
          dmem_rd_data = dmem_zero_extend ? {16'b0, mem[addr+1], mem[addr]} : {{16{mem[addr+1][7]}}, mem[addr+1], mem[addr]};
        end
